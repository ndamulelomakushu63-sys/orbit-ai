...
export default function LoginPage() {
  ...
  const handleSignIn = async (e: React.FormEvent) => {
    ...
    setTimeout(() => {
      setIsLoading(false);
      alert('Sign In clicked! Email/Password is active.');
    }, 1000);
  };
      ...
      <div className="mt-8 sm:mx-auto sm:w-full sm:max-w-md">
        <div className="bg-slate-800 py-8 px-4 shadow sm:rounded-lg sm:px-10 border border-slate-700">
          ...
          <div className="mt-6 text-center">
            <Link href="/signup" className="text-xs text-slate-400 hover:text-white font-medium">
              New to Orbit AI? <span className="text-blue-500 font-bold underline">Create accounts</span>
            </Link>
          </div>
        </div>
      </div>
